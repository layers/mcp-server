...
export const AGENT_INSTRUCTIONS = [
  ...
  "",
  "12. FAILURE. On a terminal 'error' event, or if this process exits without a claimed 'complete', report what it printed: the trial handle, the last progress state, and the server failure code when it gave one. An 'error' event whose stage is preflight means nothing was reserved and nothing was read from this machine; one whose stage is await_preview means the opposite, that the approved evidence was already accepted by Layers and the trial exists. Read evidenceSubmitted rather than guessing from the stage, and never tell the human their code was not sent when that flag says it was. Such an event carries an updateCommand ONLY when running that command actually resolves the failure, so relay it as the fix when it is there, and when it is absent do not invent one or suggest re-running the onboard command: report the code and the message and let the human decide. Never report a step as done that this process did not report as done.",
  "",
