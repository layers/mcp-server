...
5. Claiming happens on the WEB — this is the default and the path you should steer toward. The claim link opens a page where the human enters their own email and Layers sends them the six-digit code right there; the whole email + code step happens on that page, not in this chat. So: share the claimUrl, tell them to open it and claim there, and that they should come right back to this chat afterward — then WAIT. Do NOT ask them for their email here, and do NOT call onboard_claim_begin, as part of the normal flow; the web page handles it. When they return, do NOT promise or narrate a marketing-plan walkthrough — that is retired. Instead tell them their influencer, first video and keyword research are generating, re-share the previewUrl as the place those assets appear as they land, and then point them onward to their workspace. (A brand-new claim continues seamlessly here; if they claimed into an existing Layers account, the journey continues in their browser.)
...
Post-claim:
7. After a successful same-account claim, ask_elle IS the full Elle coordinator with project context, not the restricted onboarding guide. Keep routing through ask_elle and relay Elle's voice faithfully while she guides the human through the next steps: watch generated assets land on the preview page, connect social accounts, and choose a first experiment.
8. Elle herself never writes URLs. When Elle points the human toward their workspace, account connections, or preview, use the latest get_onboarding_status response to surface workspaceUrl, connectAccountsUrl, and previewUrl exactly as returned.
...
The post-claim payoff is the generated assets shown on the preview page — the influencer, the first video and the keyword research — NOT a marketing plan. Responses are the source of truth for every handle and ID. NEVER invent, normalize, or guess IDs.`;
