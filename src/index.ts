...
- START FROM THE HOST CONTEXT. If you are already in a local code workspace, call onboard_start with no URL; do not ask the human for one before reserving. If there is no viable code workspace, ask once for their public product URL, then pass exactly what they provide. NEVER guess, default, invent, or substitute a URL (including layers.ai/layers.com, your own domain, or an example URL).
- A URL-free onboard_start result with state awaiting_evidence is only a reservation. It has not collected or sent code, started a preview, or created a workspace. This package slice does not yet expose the later consented source collector, so never claim that those later steps happened and never route that reservation into the legacy preview/claim flow.
...
Flow:
1. If you are in a viable code workspace, call onboard_start with no arguments. Otherwise ask once for the human's public product URL and pass that URL to onboard_start.
2. Inspect the result. If state is awaiting_evidence, the reservation succeeded but this package has no source collector yet. Do not call ask_elle, get_onboarding_status, or either claim tool for that reservation; do not say a preview is building. Stop the legacy flow at this honest boundary.
3. Only when the explicit-URL compatibility result includes previewUrl and claimUrl, start the guided conversation through ask_elle right away, WITHOUT waiting for the build — it IS Elle's onboarding guide. Elle greets and asks the five Layers intake questions while the preview builds in the background (this both captures setup info and covers the ~1-2 min the build takes). The five intake questions are the ENTIRE pre-claim Q&A; after claim, Elle switches to experimentation and action. Route each of those turns through ask_elle: the greeting and the five intake questions. Pass the human's reply as the message. Relay Elle's response faithfully and let her voice carry the conversation — do NOT add your own greeting, re-summarize what she already said, or append your own version of her question. When Elle asks something, present it once and wait for the human; stay in the background while she drives. Do not skip questions or end a turn on a duplicate.
4. While the compatibility conversation runs, poll get_onboarding_status in the background. Do NOT share the preview link before it is ready. Once buildState is preview_ready (usually by the time the intake questions are done), share previewUrl and claimUrl and tell the human the preview link shows their brand brief.
